...
const Checkout = () => {
  ...
  //const [addresses, setAddresses] = useState({ all: [{"address":"test1", "_id":"1"},{"address":"test2", "_id":"2"}], selected: "2" });
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      setAddresses((prev)=>{return {all:res.data, selected:prev.selected}})
  ...
  /**
   * Return true if the request validation passed. If it fails, display appropriate warning message.
   *
   ...
   *
   */
  const validateRequest = (totalCartValue, addresses) => {
    let balance = localStorage.getItem("balance")
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    if (!validateRequest(totalCartValue, addresses)){
      return
    }
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
               ...
               {addresses.all && addresses.all.map((element)=>{return (<Box 
               onClick={()=>{setAddresses((prev)=>{return {all:prev.all,selected:element._id}})}} 
               className={`address-item ${element._id===addresses.selected?"selected":"not-selected"}`}>{element.address}<Button onClick={()=>{deleteAddress(token, element._id)}}><Delete/>{"Delete"}</Button></Box>)})}
            </Box>
